...
describe('Redis Comprehensive Tests', () => {
    ...
    test('CURSOR - HSCAN for Hash Fields', async () => {
        ...
        let cursor = 0;
        do {
            const result = await client.send('HSCAN', [
                hashKey,
                cursor.toString(),
                'COUNT',
                '5',
            ...
            cursor = Number(result[0]);
            const fieldValues = result[1];
            // Parse field-value pairs
            for (let i = 0; i < fieldValues.length; i += 2) {
                scannedFields[fieldValues[i]] = fieldValues[i + 1];
            }
        } while (cursor !== 0);
    ...
    test('CURSOR - SSCAN for Set Members', async () => {
        ...
        let cursor = 0;
        do {
            const result = await client.send('SSCAN', [
                setKey,
                cursor.toString(),
                'COUNT',
                '10',
            ...
            members.map((m) => scannedMembers.add(m));
        } while (cursor !== 0);
    ...
    test('CURSOR - ZSCAN for Sorted Set', async () => {
        ...
        let cursor = 0;
        do {
            const result = await client.send('ZSCAN', [
                zsetKey,
                cursor.toString(),
                'COUNT',
                '10',
            ...
            cursor = Number(result[0]);
            const memberScores = result[1];
            // Parse member-score pairs
            for (let i = 0; i < memberScores.length; i += 2) {
                scannedMembers[memberScores[i]] = memberScores[i + 1];
            }
        } while (cursor !== 0);
