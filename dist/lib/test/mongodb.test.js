...
describe('MongoDB Connector Tests', () => {
    ...
    test('FIND - With Projection and Sort', async () => {
        const result = await db
            .collection(usersCollection)
            .find({}, { projection: { username: 1, age: 1 }, sort: { age: -1 }, limit: 2 })
            .toArray();
    ...
    test('UPSERT - Update or Insert', async () => {
        ...
        // First upsert (insert)
        const result1 = await collection.updateOne({ username }, { $set: { username, email: `${username}@test.com`, age: 25 } }, { upsert: true });
        expect(result1.matchedCount + result1.upsertedCount).toBeGreaterThan(0);
        // Second upsert (update)
        const result2 = await collection.updateOne({ username }, { $set: { age: 30 } }, { upsert: true });
