...
describe('SQL Drizzle ORM Tests', () => {
    ...
    test('SELECT - With ORDER BY', async () => {
        ...
        if (!client)
            throw new Error('No client');
        const result = await client.select().from(users).orderBy(desc(users.age));
    ...
    test('SELECT - With LIMIT', async () => {
        ...
        if (!client)
            throw new Error('No client');
        const result = await client.select().from(users).limit(2);
    ...
    test('TRANSACTION - Rollback on Error', async () => {
        ...
        try {
            await client.transaction(async (tx) => {
                ...
                await tx
                    .insert(users)
                    .values({ username, email: `other_${email}`, age: 51 });
            });
        }
        catch (_error) {
            // Expected error
    ...
    test('RAW SQL - Using sql template', async () => {
        ...
        const result = await client
            .select({
            username: users.username,
            upperUsername: sql `UPPER(${users.username})`,
        })
            .from(users)
            .limit(1);
        expect(result.length).toBe(1);
    ...
    test('WINDOW FUNCTION - Using sql', async () => {
        ...
        const result = await client
            ...
            username: users.username,
            age: users.age,
            rank: sql `ROW_NUMBER() OVER (ORDER BY ${users.age} DESC)`,
        })
            .from(users);
        expect(result.length).toBeGreaterThan(0);
    ...
    test('CTE - WITH Clause', async () => {
        ...
        if (!client)
            throw new Error('No client');
        const userSpending = client.$with('user_spending').as(client
            .select({
            userId: orders.userId,
            total: sql `SUM(${orders.price}::numeric * ${orders.quantity})`.as('total'),
        })
            .from(orders)
            .groupBy(orders.userId));
        const result = await client
...
//# sourceMappingURL=sql-drizzle.test.js.map