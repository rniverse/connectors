...
describe('SQL Tool Tests', () => {
    ...
    test('INSERT', async () => {
        ...
        const result = await client.$client `INSERT INTO users (username, email, age) VALUES (${username}, ${email}, 25) RETURNING *`;
    ...
    test('UPDATE', async () => {
        ...
        if (!client)
            throw new Error('No client');
        const result = await client.$client `UPDATE users SET age = 26 WHERE username = 'alice' RETURNING *`;
    ...
    test('DELETE', async () => {
        ...
        const deleteResult = await client.$client `DELETE FROM users WHERE username = ${username} RETURNING *`;
    ...
    test('JOIN', async () => {
        ...
        if (!client)
            throw new Error('No client');
        const results = await client.$client `SELECT u.username, o.product_name FROM users u INNER JOIN orders o ON u.id = o.user_id`;
    ...
    test('AGGREGATE', async () => {
        ...
        if (!client)
            throw new Error('No client');
        const stats = await client.$client `SELECT COUNT(*) as total, AVG(price) as avg_price FROM orders`;
    ...
    test('WINDOW FUNCTION', async () => {
        ...
        if (!client)
            throw new Error('No client');
        const results = await client.$client `SELECT username, age, ROW_NUMBER() OVER (ORDER BY age DESC) as rank FROM users`;
    ...
    test('CURSOR - Simple Iteration', async () => {
        ...
        await client.transaction(async (tx) => {
            ...
            await tx.execute(sql.raw('DECLARE user_cursor CURSOR FOR SELECT * FROM users ORDER BY id'));
    ...
    test('CURSOR - Complex Query with Filter', async () => {
      ...
      `));
            ...
            for (let i = 1; i < results.length; i++) {
                expect(Number(results[i - 1].price)).toBeGreaterThanOrEqual(Number(results[i].price));
            }
            // Close cursor
    ...
    test('STREAM - Cursor-based Async Iteration', async () => {
        ...
        await client.transaction(async (tx) => {
            ...
            await tx.execute(sql.raw('DECLARE stream_cursor CURSOR FOR SELECT * FROM users ORDER BY id'));
