...
describe('MongoDB Connector Tests', () => {
  ...
  test('INSERT - Multiple Documents', async () => {
    ...
    const result = await connector.insertMany<User>(usersCollection, [
      { username: `multi_user1_${timestamp}`, email: `multi1_${timestamp}@test.com`, age: 30 },
      { username: `multi_user2_${timestamp}`, email: `multi2_${timestamp}@test.com`, age: 35 },
    ]);
  ...
  test('FIND - With Filter', async () => {
    const result = await connector.find<User>(usersCollection, { username: 'alice' });
  ...
  test('FIND - With Query Operators', async () => {
    const result = await connector.find<User>(usersCollection, { age: { $gt: 25 } });
  ...
  test('FIND - With Projection and Sort', async () => {
    ...
    if (result.data && result.data.length > 1) {
      expect(result.data[0]?.age).toBeGreaterThanOrEqual(result.data[1]?.age ?? 0);
    }
  });

  test('FINDONE - Single Document', async () => {
    const result = await connector.findOne<User>(usersCollection, { username: 'bob' });
  ...
  test('UPDATE - Single Document', async () => {
    ...
    const updated = await connector.findOne<User>(usersCollection, { username: 'alice' });
  ...
  test('DELETE - Single Document', async () => {
    ...
    const result = await connector.deleteOne<User>(usersCollection, { username });
  ...
  test('DELETE - Multiple Documents', async () => {
    const timestamp = Date.now();
    await connector.insertMany<User>(usersCollection, [
      { username: `temp1_${timestamp}`, email: `temp1_${timestamp}@test.com`, age: 99 },
      { username: `temp2_${timestamp}`, email: `temp2_${timestamp}@test.com`, age: 99 },
    ]);

    const result = await connector.deleteMany<User>(usersCollection, { age: 99 });
  ...
  test('COUNT - With Filter', async () => {
    const result = await connector.countDocuments<User>(usersCollection, { age: { $gte: 30 } });
  ...
  test('AGGREGATE - Match and Project', async () => {
    ...
    expect(result.ok).toBe(true);
    expect(result.data).toBeDefined();
    expect(result.data?.every((p: any) => p.category === 'Electronics')).toBe(true);
  });

  test('AGGREGATE - Lookup (Join)', async () => {
    // Create orders that reference users
    const usersResult = await connector.find<User>(usersCollection, { username: 'alice' });
  ...
  test('TEXT SEARCH - With Index', async () => {
    ...
    // Create text index
    await connector.createIndex(searchCollection, { name: 'text', category: 'text' });
    ...
    // Text search
    const result = await connector.find<Product>(
      searchCollection,
